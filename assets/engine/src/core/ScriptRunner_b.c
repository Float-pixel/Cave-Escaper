...
void Script_ActorSetFlip_b() { /* NOOP - This command has been removed */
  UBYTE idx, val;
  if (actor_i == 0) { //Set Bits
    actors[active_script_ctx.script_actor].palette_index = bits;
    actors[active_script_ctx.script_actor].rerender = TRUE;
  } else if (actor_i == 1) { //Set Bit
    idx = MOD_16(bits);
    val = DIV_16(bits);
    if (idx) {
      if (val) {
        SET_BIT(actors[active_script_ctx.script_actor].palette_index, idx);
      } else {
        UNSET_BIT(actors[active_script_ctx.script_actor].palette_index, idx);
      }
    } else {
      actors[active_script_ctx.script_actor].palette_index = ((actors[active_script_ctx.script_actor].palette_index) & 0xF8) | val;
    }
    actors[active_script_ctx.script_actor].rerender = TRUE;
  } else if (actor_i == 2) { //Get Bit
    if (bits) {
      bits = GET_BIT(actors[active_script_ctx.script_actor].palette_index, bits);
    } else {
      bits = MOD_8(actors[active_script_ctx.script_actor].palette_index);
    }
  }
}
...
void Script_LaunchProjectile_b() {
  ...
  if (dir_value == 1) { //Down
    ...
    ltime = 60;
    modifier = 1;
  } else if (dir_value == 2) { //Left
    dir_x = -1;
    dir_y = 0;
    ltime = 60;
    modifier = 1;
  } else if (dir_value == 4) { //Right
    dir_x = 1;
    dir_y = 0;
    ltime = 60;
    modifier = 1;
  } else if (dir_value == 8) { //Up
    dir_x = 0;
    dir_y = -1;
    ltime = 60;
    modifier = 1;
  } else if (dir_value == 11) { //EAST
    dir_x = 1;
    dir_y = 0;
    ltime = 60;
    modifier = 1;
  } else if (dir_value == 12) { //EASTNORTHEAST
    dir_x = 2;
    dir_y = -1;
    ltime = 60;
    modifier = 2;
  } else if (dir_value == 13) { //NORTHEAST
    dir_x = 1;
    dir_y = -1;
    ltime = 60;
    modifier = 1;
  } else if (dir_value == 14) { //NORTHNORTHEAST
    dir_x = 1;
    dir_y = -2;
    ltime = 60;
    modifier = 2;
  } else if (dir_value == 15) { //NORTH
    dir_x = 0;
    dir_y = -1;
    ltime = 60;
    modifier = 1;
  } else if (dir_value == 16) { //NORTHNORTHWEST
    dir_x = -1;
    dir_y = -2;
    ltime = 60;
    modifier = 2;
  } else if (dir_value == 17) { //NORTHWEST
    dir_x = -1;
    dir_y = -1;
    ltime = 60;
    modifier = 1;
  } else if (dir_value == 18) { //WESTNORTHWEST
    dir_x = -2;
    dir_y = -1;
    ltime = 60;
    modifier = 2;
  } else if (dir_value == 19) { //WEST
    dir_x = -1;
    dir_y = 0;
    ltime = 60;
    modifier = 1;
  } else if (dir_value == 20) { //WESTSOUTHWEST
    dir_x = -2;
    dir_y = 1;
    ltime = 60;
    modifier = 2;
  } else if (dir_value == 21) { //SOUTHWEST
    dir_x = -1;
    dir_y = 1;
    ltime = 60;
    modifier = 1;
  } else if (dir_value == 22) { //SOUTHSOUTHWEST
    dir_x = -1;
    dir_y = 2;
    ltime = 60;
    modifier = 2;
  } else if (dir_value == 23) { //SOUTH
    dir_x = 0;
    dir_y = 1;
    ltime = 60;
    modifier = 1;
  } else if (dir_value == 24) { //SOUTHSOUTHEAST
    dir_x = 1;
    dir_y = 2;
    ltime = 60;
    modifier = 2;
  } else if (dir_value == 25) { //SOUTHEAST
    dir_x = 1;
    dir_y = 1;
    ltime = 60;
    modifier = 1;
  } else if (dir_value == 26) { //EASTSOUTHEAST
    dir_x = 2;
    dir_y = 1;
    ltime = 60;
    modifier = 2;
  } else if (dir_value == 31) { //EASTMEDIUM
    dir_x = 1;
    dir_y = 0;
    ltime = 6;
    modifier = 2;
  } else if (dir_value == 32) { //NORTHEASTMEDIUM
    dir_x = 1;
    dir_y = -1;
    ltime = 6;
    modifier = 2;
  } else if (dir_value == 33) { //NORTHWESTMEDIUM
    dir_x = -1;
    dir_y = -1;
    ltime = 6;
    modifier = 2;
  } else if (dir_value == 39) { //WESTMEDIUM
    dir_x = -1;
    dir_y = 0;
    ltime = 6;
    modifier = 2;
  } else if (dir_value == 41) { //NORTHEASTSHORT
    dir_x = 1;
    dir_y = -1;
    ltime = 2;
    modifier = 2;
  } else if (dir_value == 49) { //NORTHWESTSHORT
    dir_x = -1;
    dir_y = -1;
    ltime = 2;
    modifier = 2;
  ...
  ProjectileLaunch(script_cmd_args[0],                         // Sprite
                   ...
                   dir_y,                                      // Dir y
                   1,                                          // Moving
                   (script_cmd_args[3] / modifier),                         // Move Speed
                   ltime,                                         // Life time
